...
function getByteEntropy(buffer) {
...
async function collectInputFiles(inputPath) {
    ...
    async function walk(dir) {
        ...
        for (const entry of entries) {
            ...
            else if (/\.(iff|cdf|bin|dat|txtr|scne|audo|raw)$/i.test(entry.name) || !path.extname(entry.name)) {
                out.push(full);
            }
        }
    }
...
function parseIffNames(iffBuffer, parsed) {
    const afterBlocks = parsed.blocks.reduce((max, block) => {
        const storedLength = block.isCompressed ? block.compressedLength : block.uncompressedLength;
        return Math.max(max, block.startOffset + storedLength);
    ...
    const offsetToNamesRaw = readUInt32LE(buf, 4);
    const offsetToNames = offsetToNamesRaw + 4 - 1;
    ...
    for (let i = 0; i < numNames && i < parsed.files.length; i++) {
        ...
        parsed.files[i].nameSource = 'iff_name_table';
        parsed.files[i].typeNameSource = 'iff_name_table';
        currentOffset += 4;
    }
...
function parseIffBuffer(buffer, baseOffset = 0) {
    ...
    for (let i = 0; i < header.blockCount; i++) {
        const nameRaw = readUInt32BE(buffer, currentOffset);
        const typeRaw = readUInt32BE(buffer, currentOffset + 4);
        const uncompressedLength = readUInt32BE(buffer, currentOffset + 12);
        const startOffset = readUInt32BE(buffer, currentOffset + 20);
        const compressedLength = readUInt32BE(buffer, currentOffset + 24);

        const block = {
            index: i,
            tableOffset: baseOffset + currentOffset,
            nameHash: toHex(nameRaw),
            nameAscii: uint32ToAscii(nameRaw),
            ...
            startOffset,
            absoluteStartOffset: baseOffset + startOffset,
            compressedLength,
            isIndexed: readUInt32BE(buffer, currentOffset + 28),
            isCompressed: compressedLength !== uncompressedLength,
            validRange: startOffset >= 0 && startOffset < buffer.length
        };
        blocks.push(block);
        currentOffset += 0x20;
    }
    ...
    for (let i = 0; i < header.fileCount && currentOffset + 12 <= header.headerSize; i++) {
        ...
        if (offsetCount > header.blockCount || currentOffset + recordLength > header.headerSize) {
            structuralWarnings.push({
                type: 'file_record_out_of_bounds',
                fileIndex: i,
                tableOffset: baseOffset + currentOffset,
                offsetCount,
        ...
        const file = {
            ...
            typeAscii: uint32ToAscii(typeRaw),
            name: String(i),
            nameSource: 'index_fallback',
            typeName: uint32ToAscii(typeRaw) || 'UNKNOWN',
            typeNameSource: uint32ToAscii(typeRaw) ? 'type_raw_ascii' : 'unknown',
            offsetCount,
            blocks: []
        };
    ...
    for (const block of blocks) {
        ...
        for (let i = 0; i < filesInBlock.length; i++) {
            ...
            const length = Math.max(0, nextRel - rel);
            file.blocks[block.index].length = length;
            ...
            file.blocks[block.index].validRange = block.startOffset + rel >= 0 && block.startOffset + rel + length <= buffer.length;
        }
    }
    ...
    const parsed = {
        ...
        blocks,
        files,
        fileOffsetTableOffset: baseOffset + fileOffsetTableOffset,
        structuralWarnings
    };
...
function parseCdfTextureRecords(buffer, baseOffset = 0) {
    ...
    while (offset + 0xB0 <= buffer.length) {
        ...
        records.push({
            ...
            recordHeaderSize,
            headerASize,
            headerBOffset: baseOffset + headerBOffset,
...
function findSignatures(buffer, maxHits) {
    const signatures = [];
    const fourccValues = KNOWN_FOURCCS.map((fourcc) => ({ fourcc: fourcc.trim(), value: fourccToUInt32(fourcc) }));

    for (let offset = 0; offset + 4 <= buffer.length; offset++) {
        if (signatures.length >= maxHits) break;
        ...
        if (be === IFF_MAGIC) {
            ...
            if (fourcc) {
                signatures.push({ kind: 'FOURCC', label: fourcc.fourcc, offset, confidence: 'raw_signature_only' });
            }
        }
    }
...
function makeNode(context, buffer, depth, manifest) {
    const sample = buffer.slice(0, Math.min(buffer.length, 0x4000));
    return {
        ...
        entropySample: getByteEntropy(sample),
        parsedIFF: null,
        cdfTextureRecords: [],
        signatures: [],
        children: []
...
async function scanBufferRecursive(buffer, context, options, manifest, depth = 0) {
    ...
    if (parsedIff) {
        ...
        for (const file of parsedIff.files) {
            const asset = {
                ...
                typeNameSource: file.typeNameSource,
                typeRaw: file.typeRaw,
                typeAscii: file.typeAscii,
                idHex: file.idHex,
            ...
            if (depth < maxDepth) {
                for (const blockRef of file.blocks) {
                    if (!blockRef.length || blockRef.length <= 0 || !blockRef.validRange) continue;
    ...
    if (cdfRecords.length > 0) {
        ...
        for (const record of cdfRecords) {
            manifest.assets.push({
                source: context.source,
                sourceRelativePath: context.sourceRelativePath,
                containerPath: `${context.containerPath}/cdf_texture_record_${record.index}`,
                typeName: 'CDF_TEXTURE_RECORD',
                typeNameSource: 'cdf_texture_record_magic',
    ...
    const signatures = findSignatures(buffer, Number(options.maxHits || DEFAULT_MAX_HITS))
...
async function writeManifest(outputPath, manifest) {
    ...
    const lines = [
        ...
        '',
        'Manifest policy:',
        '- No team IDs, roster assumptions, cyberface labels, portrait labels, or spreadsheet-derived metadata are written.',
        '- Asset names/types come only from IFF name tables, raw type fields, CDF record headers, or raw signatures.',
        '- FOURCC hits are signature evidence only, not semantic identification.'
    ];
...
module.exports = async function smartAssetScanner(inputPath, outputPath, options = {}) {
    ...
    const manifest = {
        ...
        policy: {
            evidenceOnly: true,
            noTeamIdSheet: true,
            noRosterAssumptions: true,
            noCyberfaceOrPortraitAssumptions: true,
            noTeamOwnershipLabels: true
        },
        options: {
            maxDepth: Number(options.maxDepth || DEFAULT_MAX_DEPTH),
            maxHits: Number(options.maxHits || DEFAULT_MAX_HITS),
    ...
    const inputRoot = inputStats.isDirectory() ? inputPath : path.dirname(inputPath);
    ...
    for (const filePath of files) {
        try {
            ...
            manifest.files.push({
                ...
                size: buffer.length,
                firstBytes: firstBytesHex(buffer),
                entropySample: getByteEntropy(buffer.slice(0, Math.min(buffer.length, 0x4000)))
            });
